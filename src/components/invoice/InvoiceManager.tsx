  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      if (result.create) {
        ...
        if (resulDetail.error) showAlert("error", resulDetail.error)
        else {
          setInvoiceData({
            client: null,
            items: [],
  ...
  return (
    <div className="invoice-manager">
      ...
      <div className="invoice-content-grid">
        {/* Left Panel: Client Display or Select Client Button + Add Products Button */}
        <div className="product-selection-panel">
          {invoiceData.client ? (
            <ClientDisplay client={invoiceData.client} onEdit={() => setShowClientModal(true)} />
          ) : (
            <>
              <p>Selecciona un cliente para empezar a facturar.</p>
